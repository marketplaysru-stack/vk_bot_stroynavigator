...
Картинки генерируются через нейросети (Agnes → Pixazo → Pollinations) или берутся из Pexels.
Кэш использованных картинок (по хешу) предотвращает дубли.
...
"""
    ...
    # Резервный шаблон (развёрнутый)
    return generate_template_text(topic)

def generate_template_text(topic: str) -> str:
    """
    Генерирует полноценный пост по схеме: хук, лид, тело (3-5 пунктов), вывод, CTA, темы, хештеги.
    ...
    # --- Темы для комментариев (3 вопроса) ---
    comments_themes = [
...
# ---------- ГЕНЕРАТОРЫ КАРТИНОК (с проверкой на дубли) ----------
def generate_agnes_image(prompt):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
        full_prompt = f"Professional illustration about {prompt}, construction, building, architecture, creative, modern, high quality, no people, no nature"
        data = {
            "prompt": full_prompt,
...
def generate_pollinations_image(prompt):
    try:
        seed = random.randint(1, 1000000)
        full_prompt = f"{prompt}, construction, building, architecture, creative, modern, high quality, no people, no nature"
...
def generate_image(topic):
    """
    Генерирует уникальную картинку по теме поста.
    Проверяет хеш изображения и не допускает дублей.
