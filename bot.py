...
# ----- ДИАГНОСТИКА ТОКЕНОВ (вывод в логи) -----
...
# ---------- ГЕНЕРАЦИЯ ПОЛЕЗНЫХ ПОСТОВ (строительные) ----------
def generate_useful_post(topic: str) -> str:
    hooks = [
        f"🏗️ **{topic}** – ОСНОВА ВАШЕГО ДОМА",
        f"🔨 **{topic}** – КАК НЕ ПОТЕРЯТЬ КАЧЕСТВО",
        f"🏠 **{topic}** – ВАШ ДОМ ЗАСЛУЖИВАЕТ ЛУЧШЕГО",
        f"😤 **{topic}** – ОШИБКИ, КОТОРЫЕ ДОРОГО СТОЯТ",
        f"🚀 **{topic}** – ТЕХНОЛОГИИ, КОТОРЫЕ МЕНЯЮТ ИГРУ",
        f"🔥 **{topic}** – АКТУАЛЬНЫЕ СОВЕТЫ ДЛЯ СТРОИТЕЛЕЙ",
        f"💡 **{topic}** – ИНСАЙТЫ ОТ ПРОФЕССИОНАЛОВ",
        f"📐 **{topic}** – ПРАВИЛЬНАЯ ПЛАНИРОВКА – ЗАЛОГ УЮТА"
    ]
...
# ---------- ИГРОВЫЕ ФОРМАТЫ (строительные) ----------
def generate_construction_quiz(topic):
    questions = [
        {"q": "Какой материал лучше всего подходит для несущих стен в малоэтажном строительстве?", "options": ["Газобетон", "Кирпич", "Дерево", "Сэндвич-панели"], "answer": "Газобетон"},
        {"q": "Что такое «монолитное строительство»?", "options": ["Строительство из блоков", "Заливка бетона в опалубку", "Каркасное строительство", "Строительство из панелей"], "answer": "Заливка бетона в опалубку"},
        {"q": "Какой материал лучше всего удерживает тепло?", "options": ["Минеральная вата", "Пенопласт", "Эковата", "Пенополиуретан"], "answer": "Пенополиуретан"},
        {"q": "Что такое «усадка дома» и когда она происходит?", "options": ["В первый год после строительства", "Через 5 лет", "Никогда", "Только зимой"], "answer": "В первый год после строительства"},
        {"q": "Какая минимальная толщина утеплителя для стен в средней полосе России?", "options": ["50 мм", "100 мм", "150 мм", "200 мм"], "answer": "150 мм"},
        {"q": "Какой тип фундамента самый надёжный для пучинистых грунтов?", "options": ["Ленточный мелкозаглублённый", "Свайно-винтовой", "Плитный", "Столбчатый"], "answer": "Свайно-винтовой"},
        {"q": "Что такое «сухая стяжка пола»?", "options": ["Стяжка без воды", "Стяжка из гипса", "Стяжка из цемента", "Стяжка с подогревом"], "answer": "Стяжка без воды"},
        {"q": "Какая кровля считается самой долговечной?", "options": ["Металлочерепица", "Гибкая черепица", "Керамическая черепица", "Шифер"], "answer": "Керамическая черепица"}
    ]
...
def create_post_content(title, text=None):
    if text and len(text) > 50:
        ...
        if AGNES_API_KEY:
            try:
                headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
                prompt = f"Перепиши следующий текст на чистом русском языке, без ошибок, сделай его полезным и структурированным для строительной тематики. Текст:\n\n{text}"
                data = {
                    "model": "agnes-v1",
...
# ---------- ЗАПУСК ----------
def main():
    logger.info("🚀 Строительный навигатор запущен (анонсы отключены)")
